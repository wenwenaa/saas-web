<template>
  <div>
    <tzHeadline :title="type | filterDic(dic.wareInStatus,'type','order')" isShowBack>
      <template slot="btn">
        <el-button class="tz-search-submit" round size="medium" v-show="isShowPrint" @click="openPrint">打印</el-button>
      </template>
    </tzHeadline>
    ...
    <el-row>
      <el-col :span="6">
        <div class="d-flex contentColor mb20">
          <div class="labelColor label80 flex-shrink-0">入库单号：</div>
          <tzToolTip :content="detail.inCode"></tzToolTip>
        </div>
      </el-col>
      <el-col :span="6">
        <div class="d-flex contentColor mb20">
          <div class="labelColor label80 flex-shrink-0">入库类型：</div>
          <tzToolTip :content="detail.inType| filterDic(dic.wareInStatus,'type','name')"></tzToolTip>
        </div>
      </el-col>
      <el-col :span="6">
        <div class="d-flex contentColor mb20">
      ...
      <el-col :span="6">
        <div
          class="d-flex contentColor mb20"
          v-if="type==='PURCHASE'||type==='OTHER_IN'||type==='SUPPLY'"
        >
          <div class="labelColor label80 flex-shrink-0">供应商：</div>
          <tzToolTip :content="detail.baseSupplierName"></tzToolTip>
        </div>
        <div class="d-flex contentColor mb20" v-if="type==='SALE_RETURN'">
    ...
    <tzCommonFileUpload
      ...
      v-if="detail.fileList&&detail.fileList.length"
    />
    <div class="section-title" style="margin-top:20px;">产品信息</div>
    <el-table
      :data="materialList"
    ...
    >
      ...
      <el-table-column label="质检情况" align="center" v-if="type!=='EXCHANGE_IN'">
        <template
          slot-scope="scope"
        >{{scope.row.inCheckFlag==='Y'?(scope.row.checkFlag==='Y'?'已质检':'未质检'):'无需质检'}}</template>
      </el-table-column>
      <el-table-column prop="defaultWarehouseName" label="入库仓库" width="180" align="center"></el-table-column>
      <el-table-column prop="quantity" label="入库数量" width="220" align="center"></el-table-column>
      <el-table-column prop="unitName" label="单位" align="center"></el-table-column>
      <el-table-column
        prop="refCode"
        :label="type|filterDic(dic.tipStatus,'type','name')"
        width="220"
        align="center"
        v-if="type!=='OTHER_IN'&&type!=='EXCHANGE_IN'&&type!=='INIT'"
      >
        <template slot-scope="scope">
          <div class="yellow" @click="toOrderDetail(scope.row)">{{scope.row.refCode}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="managerStuff" label="入库人" width="220" align="center">
        <template slot-scope="scope">
          <div >{{scope.row.managerStuff || '-'}}</div>
        </template>
      </el-table-column>
      <el-table-column label="入库时间" align="center" :width="type==='EXCHANGE_IN'?'':'180'">
    ...
    <div class="d-flex justify-between" style="width:500px;" v-if="detail.checkStatus!=='NOT'">
      ...
      <div class="check-sec">
        <span class="sum">{{detail.checkMaterialList&&detail.checkMaterialList.length}}条</span>需要质检
      </div>
    </div>
    <tzSteps2
      :stepsList="dic.qualityStatus2"
      titleName="name"
      :activeVal="detail.checkStatus"
      v-if="detail.checkStatus!=='NOT'"
    ></tzSteps2>
    <div class="section-title">入库情况</div>
    <tzSteps2 :stepsList="dic.completionStatus1" titleName="name" :activeVal="detail.pushStatus"></tzSteps2>
    <div class="section-title" v-if="detail.checkStatus!=='NOT'">质检</div>
    <el-table
      :data="detail.checkMaterialList"
      border
      style="width: 100%; margin-top: 20px;margin-bottom:20px;"
      :header-cell-style="{'background-color': '#F9F9F9'}"
      v-if="detail.checkStatus!=='NOT'"
    >
      ...
      <el-table-column prop="materialName" label="物料名称" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column prop="spec" label="物料规格" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column prop="checkQuantity" label="检验数量" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.result==='U'?'-':scope.row.goodQuantity+scope.row.ngQuantity||0}}</template>
      </el-table-column>
      <el-table-column prop="goodQuantity" label="良品数" align="center">
        <template slot-scope="scope">{{scope.row.result==='U'?'-':scope.row.goodQuantity||0}}</template>
      </el-table-column>
      <el-table-column prop="ngQuantity" label="不良品数" align="center">
        <template slot-scope="scope">{{scope.row.result==='U'?'-':scope.row.ngQuantity||0}}</template>
      </el-table-column>
      ...
      <el-table-column prop="formCode" label="质检单号" width="220" align="center">
        <template slot-scope="scope">
          <div class="yellow" @click="toCheckDetail(scope.row)">{{scope.row.formCode}}</div>
        </template>
      </el-table-column>
    </el-table>
     <!-- 打印组件 -->
    <tzPrint
      :orders="[{ code: detail.inCode, id: detail.stockInId }]"
...
export default {
  ...
  computed: {
    ...mapState({
      detail: state => state.ware.detail
    }),
    isShowPrint () {
      const arr = ['EXCHANGE_IN', 'INIT']
      return !arr.includes(this.detail.inType)
    }
  },
  created () {
    this.stockInId = this.$route.params.id
    this.type = this.$route.query.type
    document.title = this.dic.wareInStatus.filter(
      item => item.type === this.type
    )[0].order
    this.getDetail()
  },
  methods: {
    ...mapActions(['A_GET_DETAIL']),
    toOrderDetail (row) {

  ...
  <div class="wareHouseIn" ref="mainContainer">
    <tzHeadline title="采购申请单">
      <template slot="btn">
        ...
        <tzBtn icon="tz-icon-genera" @click="generateOrder">生成采购订单</tzBtn>
      </template>
    </tzHeadline>
    ...
    >
      ...
      <el-table-column label="申请人员" prop="managerStuff" align="center"></el-table-column>
      ...
      <el-table-column label="转单状态" align="center">
        <template slot-scope="scope">
        ...
        <template slot-scope="scope">
          <div class="btn">
            ...
            <el-button
              ...
              type="text"
              class="gray f14"
              @click="addBtn('detail',scope.row)"
            >详情</el-button>
            <el-button
              size="small"
              type="text"
              class="yellow f14"
              @click="delBtn(scope.row.purchaseApplyId)"
            >删除</el-button>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...mapState({
      total: state => state.purchase.purchaseObj.allTotal,
      tableLoading: state => state.global.tableLoading
    })
  },
  methods: {
    // 所有异步请求
    ...mapActions(['GET_PURCHASE_ALL_LIST', 'APPROVELIST_PURCHASE_TYPE', 'POST_GLOBAL_AUDITOR', 'DELETE_PURCHASE_TYPE']),
    // 同步vuex数据
    ...mapMutations([]),
    // 设置请求参数
    setParams () {
    ...
    async getData () {
      ...
      const { status, data } = await this.GET_PURCHASE_ALL_LIST(arg)
      if (status === 0) {

  ...
  <div class="addPage">
    <tzHeadline :title="title" backText="返回" :isShowBack="isShowBack">
      <template slot="btn">
        ...
        <div v-else>
          <el-button round size="medium" class="tz-btn-add" @click="applySave(routeParame.type,routeParame.page)" v-if="detailObj.status != 'SUBMIT' && detailObj.status !== 'APPROVED' && detailObj.status !== 'CLOSED'">编辑</el-button>
          <el-button round size="medium" class="tz-search-submit tz-btn-add" @click="approveBtn(detailObj)" v-if="detailObj.status != 'SUBMIT' && detailObj.status !== 'APPROVED'&& detailObj.status !== 'CLOSED'">送审</el-button>
        </div>
      </template>
    </tzHeadline>
    ...
    <div class="section2 mt20">
      <div class="section-label" v-if="routeParame.page !== 'none'">关联单据</div>
    ...
    <el-table :data="finishedObj.productData" border style="width: 100%; margin-top: 20px"
      :header-cell-style="{'background-color': '#F9F9F9'}" v-if="routeParame.type=='add'&&routeParame.page=='finished'">
      ...
      <el-table-column prop="quantityDisplay" label="采购申请数量" width="220" align="center" class-name="required-red addEdit">
      ...
      <el-table-column label="交期" width="220" align="center" class-name="required-red">
        <template slot-scope="scope">
          <el-date-picker v-model="scope.row.sendDate" type="date" placeholder="请选择" clearable></el-date-picker>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" fixed="right">
        <template slot-scope="scope">
          <div class="btn">
            <el-button size="small" type="text" class="yellow f14" @click="delBtn(scope.$index)">删除
            </el-button>
          </div>
        </template>
      </el-table-column>
    ...
    <div
      ...
      <span class="lf-title">产品信息</span>
      <span class="rf-material">
        <el-button round size="medium" class="tz-search-submit mr20" @click="visibleMaterial = true" v-if="routeParame.page === 'none'">添加物料</el-button>
        <el-button round size="medium" class="tz-search-submit mr20" @click="setReset" v-else>重置</el-button>
        <el-date-picker type="date" v-model="deliveryTime" placeholder="设置默认交期" @change="setDate"></el-date-picker>
      </span>
    </div>
    <el-table :span-method="cellMerge1" :data="finishedObj.productData" border style="width: 100%; margin-top: 20px"
      :header-cell-style="{'background-color': '#F9F9F9'}" v-if="routeParame.type=='edit'&&routeParame.page=='finished'||routeParame.type!='detail'&&routeParame.page=='none'">
      <el-table-column type="index" label="序号" width="50" align="center" fixed="left">
      </el-table-column>
      ...
      <el-table-column prop="saleCode" label="销售订单号" width="180" align="center" v-if="routeParame.page === 'finished'">
        <template slot-scope="scope">
          <div class="yellow" @click="goSale(scope.row.saleOrderId)">{{scope.row.saleCode}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="buyQuantity" label="订单数量" width="180" align="center" v-if="routeParame.page === 'finished'">
        <template slot-scope="scope">
          {{scope.row.buyQuantity}}
        </template>
      </el-table-column>
      <el-table-column prop="referenceDisplay" label="采购参考数量" width="180" align="center" v-if="routeParame.page === 'finished'">
        <template slot-scope="scope">
    ...
    <el-table :data="finishedObj.productData" border style="width: 100%; margin-top: 20px"
      :header-cell-style="{'background-color': '#F9F9F9'}" v-if="routeParame.type === 'add' && routeParame.page === 'subpart'">
      <el-table-column type="index" label="序号" width="50" align="center" fixed="left">
        <template slot-scope="scope">
          0{{scope.$index+1}}
        </template>
      ...
      <el-table-column prop="saleCode" label="销售订单号" align="center">
          <template slot-scope="scope">
            <div class="yellow">{{scope.row.saleCode}}</div>
          </template>
      </el-table-column>
    ...
    <div class="section-label mt20" v-if="(routeParame.type === 'add' || routeParame.type === 'edit') && routeParame.page === 'subpart'">
      ...
      <span class="rf-material">
        ...
        <el-date-picker v-model="deliveryTime" type="date" placeholder="设置默认交期" @change="setDate"></el-date-picker>
      </span>
    </div>
    <el-table :data="finishedObj.productData" border style="width: 100%; margin-top: 20px"
    ...
    <el-table
      ...
      v-if="routeParame.type === 'detail' && routeParame.page !== 'none'">
      <el-table-column type="index" label="序号" width="50" align="center" fixed="left">
        <template slot-scope="scope">
          0{{scope.$index+1}}
        </template>
    ...
    <el-table :data="detailObj.relationPurchaseOrderList" border style="width: 100%; margin-top: 20px"
      ...
      <el-table-column prop="purchaseCode" label="采购订单号" align="center">
          <template slot-scope="scope">
            <el-button size="small" type="text" class="yellow" @click="goTo(scope.row.purchaseOrderId)">{{scope.row.purchaseCode}}</el-button>
          </template>
      </el-table-column>
    </el-table>
    <material
...
export default {
  ...
  created () {
    ...
    if (Number(this.routeParame.id) !== 0) {
  ...
  methods: {
    // 所有异步请求
    ...mapActions(['DETAIL_FINISHED_TYPE', 'ADD_PURCHASE_FINISHED_TYPE', 'POST_GLOBAL_AUDITOR', 'APPROVESEND_PURCHASE_TYPE', 'DETAIL_SUBPART_TYPE', 'ADD_PURCHASE_NON_FINISHED_TYPE', 'DETAIL_APPLY_TYPE', 'EDIT_APPLY_TYPE', 'OPEN_PURCHASE_TYPE', 'CLOSE_PURCHASE_TYPE', 'APPROVELIST_PURCHASE_TYPE', 'POST_GLOBAL_RECALL_AUDIT', 'ADD_PURCHASE_TYPE', 'DELETE_PURCHASE_TYPE']),
    // 同步vuex数据
    ...mapMutations([]),
    // 设置请求参数
    setParams () {
      const materialList = this.finishedObj.productData.map(item => ({
        ...item,
    ...
    async getFinisheDetail () {
      ...
      if (status === 0) {
        ...
        this.finishedObj.productData = data.purchaseApplyMaterialVoList
        if (this.routeParame.page === 'subpart' && this.routeParame.type === 'edit') {
          this.finishedObj.needData = data.purchaseApplyMaterialVoList
        }
        this.spanArr = []
        if (this.routeParame.page === 'finished' && (this.routeParame.type === 'detail' || this.routeParame.type === 'edit')) {
    ...
    async getNoneAdd (val, type) {
      ...
      const arg = this.setParams()
      const { status, data } = await this.ADD_PURCHASE_TYPE(arg)
      if (status === 0 && type === 'auditor') {
        this.approveFn(data)
      }
    },
    ...
    async sendCheck (item) {
      const materialList = item.purchaseApplyMaterialVoList.map(it => ({
    ...
    submitForm (formName, type) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.routeParame.page === 'finished' && this.routeParame.type === 'add' && type === 'none') {
            // 成品新增
            this.getAdd(type)
          } else if (this.routeParame.page === 'subpart' && this.routeParame.type === 'add' && type !== 'auditor') {
            this.getNonAdd(type)
          } else if (this.routeParame.page === 'none' && this.routeParame.type === 'add' && type === 'none') {
            // 无来源新增
            this.getNoneAdd(type)
          } else if (this.routeParame.page === 'none' && this.routeParame.type === 'edit' && type === 'none') {
            // 无来源编辑
            this.getEdit(type)
          } else if ((this.routeParame.page === 'finished' || this.routeParame.page === 'subpart') && type === 'none' && type !== 'auditor') {
            this.getEdit(type)
          } else if (this.routeParame.page === 'finished' && type === 'auditor') {
            this.sendCheck(this.detailObj)
          } else if (this.routeParame.page === 'subpart' && type === 'auditor') {
            this.getNonAdd(this.detailObj)
          } else if (this.routeParame.page === 'none' && type === 'auditor') {
            this.getNoneAdd(this.detailObj, type)
          }
        }
      })
    },
    ...
    // 删除
    delBtn (type, index) {
      console.log(this.finishedObj.productData)
    ...
    async materialSubmit (val) {
      try {
        const copyVal = val.map(item => ({
          ...item,
          minSafetyStockQuantity: item.minSafetyStock,
          materialType: item.typeName
        }))
        this.finishedObj.productData = [...this.finishedObj.productData, ...copyVal]
        this.spanArr = []
        this.getSpanArr(this.finishedObj.productData)
        this.close()
      } catch (e) {
    ...
    checkTime (e) {
      ...
      if (page === 'none' && type === 'add') return
      if (type === 'edit' && page === 'finished') {

<template>
  <div class="contain-company" ref="rootPlacement">
    ...
    <tzHeadline title="销售订单">
      <template slot="btn">
        <el-button class="tz-btn-add" round size="medium" @click="goTo('add')">新增订单</el-button>
    ...
    >
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          ...
          >编辑</span>
          <span class="mr10 yellow" @click="goTo('detail', scope.row.saleOrderId)">详情</span>
          <span
            class="yellow"
            v-if="scope.row.status === 'DRAFT'"
...
export default {
  ...
  computed: {
    ...mapState({
      // total: state => state.sales.sales_list.total,
      // tableData: state => state.sales.sales_list.salesList
  ...
  methods: {
    ...
    async getSalelist () {
      try {
        ...
        this.expandText = '全部展开'
        const { status, data } = await this.GET_SALES_SINGLE_LIST(
          this.formData
        )
        if (status === 0) {

<template>
  <div class="contain-company">
    <!-- 头部 -->
    <tzHeadline title="物料质检方案详情" :isShowBack="true" backText="返回">
      <template slot="btn">
    ...
    <el-form
      ...
      label-position="top">
        ...
        <el-table :data="detail.checkPlanRangeVos" border style="width: 100%;" class="spotCheck" :header-cell-style="{'background-color': '#F9F9F9'}" v-if="detail.rangeFlag === 'Y'">
        ...
        <div class="detailList" v-if="detail.specFlag === 'Y'">
          <el-row :gutter="20">
            <el-col :span="5">
              <div class="item" style="display:flex;margin-bottom: 0">检验方法：<tzToolTip :content="detail.specMethod" activeColor="#444" /></div>
            </el-col>
            <el-col :span="5">
              <div class="item" style="display:flex;margin-bottom: 0">检验工具：<tzToolTip :content="detail.specTool" activeColor="#444" /></div>
            </el-col>
          </el-row>
        </div>
        ...
        <el-table :data="detail.checkPlanOtherVos" border style="width: 100%;" class="clearInput" :header-cell-style="{'background-color': '#F9F9F9'}" v-if="detail.otherFlag === 'Y'">
...
export default {
  ...
  methods: {
    ...
    // 获取详情
    async getGlobalQuality () {
      try {
        const { status, data } = await this.A_GET_MATERIAL_QUALITY_DETAIL({ checkPlanId: this.list.checkPlanId })
        if (status === 0) {
          this.detail = data
        }
      } catch (e) {

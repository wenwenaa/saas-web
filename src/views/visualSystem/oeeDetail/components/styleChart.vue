<template>
	<div class="single-style-chart-container">
			...
			calcChartData () {
				...
				for (let i = 0; i < this.classCount; i += 1) {
					...
					this.oeeList.forEach((item) => {
						...
						// ret[0].tooltip.formatter = `${i+1}班<br />运行：${item.runTime}H 20%<br />暂停：${item.pauseTime}H 20%<br />故障：${item.faultTime}H 20%<br />离线：${item.offTime}H 20%`
						// ret[1].tooltip.formatter = `${i+1}班<br />运行：${item.runTime}H 20%<br />暂停：${item.pauseTime}H 20%<br />故障：${item.faultTime}H 20%<br />离线：${item.offTime}H 20%`
						// ret[2].tooltip.formatter = `${i+1}班<br />运行：${item.runTime}H 20%<br />暂停：${item.pauseTime}H 20%<br />故障：${item.faultTime}H 20%<br />离线：${item.offTime}H 20%`
						// ret[3].tooltip.formatter = `${i+1}班<br />运行：${item.runTime}H 20%<br />暂停：${item.pauseTime}H 20%<br />故障：${item.faultTime}H 20%<br />离线：${item.offTime}H 20%`
						if (i === 0) {
							this.xAxisData.push(item.hisDate)
						}
					})
				...
				return [
					{
						...
						tooltip: {
							...tooltip,
							formatter: `${i + 1}班<br />运行：1H 20%<br />暂停：2H 20%<br />故障：3H 20%<br />离线：4H 20%`
						}
					},
					{
						name: '暂停',
